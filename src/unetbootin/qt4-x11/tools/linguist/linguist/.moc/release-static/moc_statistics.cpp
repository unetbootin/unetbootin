/****************************************************************************
** Meta object code from reading C++ file 'statistics.h'
**
** Created: Thu Mar 5 20:56:06 2009
**      by: The Qt Meta Object Compiler version 61 (Qt 4.5.0)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../statistics.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'statistics.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 61
#error "This file was generated using the moc from 4.5.0. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_Statistics[] = {

 // content:
       2,       // revision
       0,       // classname
       0,    0, // classinfo
       2,   12, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors

 // slots: signature, parameters, type, tag, flags
      32,   12,   11,   11, 0x0a,
      69,   11,   11,   11, 0x09,

       0        // eod
};

static const char qt_meta_stringdata_Statistics[] = {
    "Statistics\0\0w1,c1,cs1,w2,c2,cs2\0"
    "updateStats(int,int,int,int,int,int)\0"
    "languageChange()\0"
};

const QMetaObject Statistics::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_Statistics,
      qt_meta_data_Statistics, 0 }
};

const QMetaObject *Statistics::metaObject() const
{
    return &staticMetaObject;
}

void *Statistics::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_Statistics))
        return static_cast<void*>(const_cast< Statistics*>(this));
    if (!strcmp(_clname, "Ui::Statistics"))
        return static_cast< Ui::Statistics*>(const_cast< Statistics*>(this));
    return QDialog::qt_metacast(_clname);
}

int Statistics::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: updateStats((*reinterpret_cast< int(*)>(_a[1])),(*reinterpret_cast< int(*)>(_a[2])),(*reinterpret_cast< int(*)>(_a[3])),(*reinterpret_cast< int(*)>(_a[4])),(*reinterpret_cast< int(*)>(_a[5])),(*reinterpret_cast< int(*)>(_a[6]))); break;
        case 1: languageChange(); break;
        default: ;
        }
        _id -= 2;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
