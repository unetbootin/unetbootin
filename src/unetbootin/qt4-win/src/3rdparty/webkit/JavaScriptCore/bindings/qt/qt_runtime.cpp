...
static JSRealType valueRealType(ExecState* exec, JSValue* val)
{
    if (val->isNumber())
        ...
        if (object->inherits(&KJS::RuntimeArray::info))  // RuntimeArray 'inherits' from Array, but not in C++
            return RTArray;
    ...
    switch (hint) {
        ...
        case QMetaType::QVariantMap:
            if (type == Object || type == Array || type == RTArray) {
                ...
                PropertyNameArray::iterator it = properties.begin();
        ...
        case QMetaType::QVariantList:
            if (type == RTArray) {
                ...
                QVariant var = convertValueToQVariant(exec, value, QMetaType::Void, &objdist);
                if (objdist != -1) {
                    QVariantList result;
                    result << var;
                    ret = QVariant(result);
...
JSValue* convertQVariantToValue(ExecState* exec, PassRefPtr<RootObject> root, const QVariant& variant)
{
    ...
    if (type == QMetaType::QDateTime ||
        ...
        DateObjectImp *dateObj = static_cast<DateObjectImp*>(exec->lexicalInterpreter()->builtinDate());
        List args;
...
JSValue *QtRuntimeConnectionMethod::callAsFunction(ExecState *exec, JSObject *, const List &args)
{
    ...
    if (sender) {
        ...
        JSObject *thisObject = exec->lexicalInterpreter()->globalObject();
        ...
        if (signalIndex != -1) {
            ...
            if (d->m_isConnect) {
                ...
                if (!ok) {
                    delete conn;
                    QString msg = QString(QLatin1String(
...
void QtConnectionObject::execute(void **argv)
{
    ...
    if (obj) {
        ...
        if (ro) {
            Interpreter* interp = ro->interpreter();
            if (interp) {
