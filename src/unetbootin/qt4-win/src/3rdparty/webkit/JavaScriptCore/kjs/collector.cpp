...
// pthread_getattr_np or pthread_attr_getstack.
    ...
    (__UCLIBC_MINOR__ < 9 || \
     (__UCLIBC_MINOR__ == 9 && __UCLIBC_SUBLEVEL__ < 29))
...
static inline void* currentThreadStackBase()
{
    ...
    thr_stksegment(&s);
    return s.ss_sp;
#elif PLATFORM(HPUX_OS)
    _pthread_stack_info_t stackInfo;
    pthread_t thread = pthread_self();
    _pthread_stack_info_np(thread, &stackInfo);
    return stackInfo.stk_stack_base;
#elif PLATFORM(UNIX)
