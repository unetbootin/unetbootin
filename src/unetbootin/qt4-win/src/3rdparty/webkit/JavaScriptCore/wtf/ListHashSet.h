...
namespace WTF {
    ...
    inline typename ListHashSet<T, U>::iterator ListHashSet<T, U>::find(const ValueType& value)
    {
        ...
        WTF_TYPENAME ImplType::iterator it = m_impl.template find<ValueType, Translator>(value);
        if (it == m_impl.end())
            return end();
    ...
    {
        typedef ListHashSetTranslator<ValueType, HashFunctions> Translator;
        WTF_TYPENAME ImplType::const_iterator it = m_impl.template find<ValueType, Translator>(value);
        if (it == m_impl.end())
            return end();
