...
static QTextLine setupLayout(QTextLayout *layout, const TextStyle &style)
{
...
void Font::drawText(GraphicsContext* ctx, const TextRun& run, const TextStyle& style, const FloatPoint& point, int from, int to) const
{
    ...
    if (from > 0 || to < run.length()) {
        ...
        QTextLine line = setupLayout(&layout, style);
    ...
    int flags = style.rtl() ? Qt::TextForceRightToLeft : Qt::TextForceLeftToRight;
    p->drawText(pt, string, flags, style.padding());
}
...
int Font::offsetForPosition(const TextRun& run, const TextStyle& style,
                            int position, bool /*includePartialGlyphs*/) const
{
    QString string = qstring(run);
    QTextLayout layout(string, m_font);
    QTextLine line = setupLayout(&layout, style);
