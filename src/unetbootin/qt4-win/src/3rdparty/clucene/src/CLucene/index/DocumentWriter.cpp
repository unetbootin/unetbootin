...
//Post - Instance has been created
	...
	term = _CL_POINTER(t);
	positions.values = _CL_NEWARRAY(int32_t,1);
	positions.values[0] = position;
	positions.length = 1;
	
	if ( offset != NULL ){
		this->offsets.values = _CL_NEWARRAY(TermVectorOffsetInfo,1);
...
void DocumentWriter::addDocument(const char* segment, Document* doc) {
	...
	fieldOffsets = _CL_NEWARRAY(int32_t,fieldInfos->size());    // init fieldOffsets
...
void DocumentWriter::addPosition(const TCHAR* field,
	...
	if (ti != NULL) {				  // word seen before
		...
		if (ti->positions.length == freq) {	  // positions array is full
			int32_t* positions = ti->positions.values;
			int32_t* newPositions = _CL_NEWARRAY(int32_t,freq * 2);	  // double size
			memcpy(newPositions,positions, freq * sizeof(int32_t)); // copy old positions to new
			_CLDELETE_ARRAY( ti->positions.values );
			ti->positions.values = newPositions;
		...
		if (offset != NULL) {
			if (ti->offsets.length == freq){
				TermVectorOffsetInfo* offsets = ti->offsets.values;
				TermVectorOffsetInfo* newOffsets = _CL_NEWARRAY(TermVectorOffsetInfo,freq*2);
				memcpy(newOffsets,offsets,freq * sizeof(TermVectorOffsetInfo));
				ti->offsets.values = newOffsets;
				_CLDELETE_ARRAY(offsets);
			}
